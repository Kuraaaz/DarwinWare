...
NTSTATUS io_device_control(PDEVICE_OBJECT device, PIRP irp) {
	...
	case ioctl_allocate_virtual_memory: {
		...
		if (NT_SUCCESS(status)) {
			...
			status = ZwAllocateVirtualMemory(NtCurrentProcess(), &in->addr, 0, &in->size,
	...
	case ioctl_protect_virutal_memory: {
		...
		if (NT_SUCCESS(status)) {
			...
			KeStackAttachProcess(target_proc, &apc);
			status = ZwProtectVirtualMemory(NtCurrentProcess(), &in->addr, &in->size, in->protect, &old_protection);
			KeUnstackDetachProcess(&apc);
	...
	case ioctl_write_shellcode: {
		...
		if (NT_SUCCESS(status)) {
			...
			status = copy_memory(PsGetCurrentProcess(), target_proc, (PVOID)in->src, (PVOID)in->dst, in->size);
		...
		PIO_STACK_LOCATION stack = IoGetCurrentIrpStackLocation(irp);
		if (irp->UserBuffer && stack->Parameters.DeviceIoControl.OutputBufferLength >= sizeof(NTSTATUS)) {
			*(NTSTATUS*)irp->UserBuffer = status;
	...
	case ioctl_create_thread: {
		pk_create_thread_request in = (pk_create_thread_request)irp->AssociatedIrp.SystemBuffer;
		PEPROCESS target_proc;
		...
		if (NT_SUCCESS(status)) {
			...
			if (NT_SUCCESS(status)) {
				// Version alternative sans PUSER_THREAD_START_ROUTINE
				status = RtlCreateUserThread(
					...
					0,                           // StackReserve  
					0,                           // StackCommit
					(PVOID)(ULONG_PTR)in->start_address,     // StartAddress
					(PVOID)(ULONG_PTR)in->parameter,         // StartParameter
					&hThread,                   // ThreadHandle
					&clientId                   // ClientID
				);

				if (NT_SUCCESS(status)) {
					DbgPrint("[DRIVER] Thread créé avec succès\n");
					ZwClose(hThread);
				}
	...
	case ioctl_find_thread: {
		...
		if (NT_SUCCESS(status)) {
			status = FindSuitableThread(target_proc, &in->thread_id);
			ObfDereferenceObject(target_proc);
		}
		info_size = sizeof(k_find_thread_request);
	} break;
